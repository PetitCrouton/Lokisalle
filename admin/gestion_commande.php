...
if(isset($_GET['action']) && $_GET['action'] == 'suppression' && !empty($_GET['id_commande']) && is_numeric($_GET['id_commande']))
{
	...
	$commande_a_supprimer = $pdo->prepare("SELECT * FROM commande WHERE id_commande = :id_commande");
	$commande_a_supprimer->bindParam(":id_commande", $id_commande, PDO::PARAM_STR);
	$commande_a_supprimer->execute();
	
	$commande_a_suppr = $commande_a_supprimer->fetch(PDO::FETCH_ASSOC);
	...
	$suppression->execute();
	$message .= '<div class="alert alert-success" role="alert" style="margin-top: 20px;">La commande numéro ' . $id_commande . ' a bien été supprimé</div>';
...
?>
    ...
    <div class="container">
      ...
      <div class="starter-template">
        <h1><span class="glyphicon glyphicon-download-alt" style="color:plum;"></span>Gestion des commandes</h1>
        <hr/>
        ...
        <?php
          ...
          echo 'Nombre de commande présentes dans la base:' . " " .$resultat->rowCount();
        ...
        while($ligne = $resultat->fetch(PDO::FETCH_ASSOC))
        {
            ...
            echo '<a onclick="return(confirm(\'Etes vous sûr\'));" href="?action=suppression&id_commande=' . $ligne['id_commande'] . '" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></a></td>';
